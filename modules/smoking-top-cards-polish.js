...
@media(max-width:380px){#screen-smoking .hf-smoking-actions{grid-template-columns:1fr}}
  ...
  function ring() {
    ...
    if (!svg) {
      ...
      svg.innerHTML = `<circle class="hf-smoke-progress-bg" cx="110" cy="110" r="${R}"></circle><circle class="hf-smoke-progress-value" cx="110" cy="110" r="${R}"></circle>`;
      box.prepend(svg);
    }
    const progress = $('.hf-smoke-progress-value', svg);
    if (progress) progress.style.strokeDashoffset = String(C * (1 - metrics().progress));
  }
