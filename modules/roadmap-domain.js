(function(root) {
  ...
  function build(snapshot,goals,today=key(new Date().toISOString())) {
    const index=indexEntries(snapshot.entries||[],snapshot.habits||[],today), out=[], projects=new Map((snapshot.projects||[]).filter(p=>!p.is_archived).map(p=>[p.id,p]));
    for(const g of goals){if(g.isArchived)continue;const m=g.metadata||{};out.push({id:g.id,source:'goal',group:m.category||'Meine Ziele',title:g.title,start:m.startDate,end:m.dueDate,...progress(g,index,today),goal:g});}
    for(const p of projects.values())out.push({id:p.id,source:'project',group:'Projekte',title:p.title,start:key(p.start_date),end:key(p.end_date),label:p.status==='done'?'Abgeschlossen':'Projekt'});
    for(const m of snapshot.milestones||[]){if(m.is_archived||!projects.has(m.project_id))continue;out.push({id:m.project_id,source:'project',group:'Projekte',title:m.title,start:key(m.milestone_date),end:key(m.milestone_date),label:projects.get(m.project_id).title});}
    for(const t of snapshot.tasks||[]){if(t.is_archived)continue;out.push({id:t.id,source:'task',group:'Aufgaben',title:t.title,start:key(t.due_at),end:key(t.due_at),label:t.status==='done'?'Erledigt':projects.get(t.project_id)?.title||'Aufgabe'});}
    for(const a of snapshot.appointments||[]){if(a.is_archived)continue;out.push({id:a.id,source:'appointment',group:'Termine',title:a.title,start:key(a.starts_at),end:key(a.ends_at)||key(a.starts_at),label:'Termin'});}
    return out.map((row,index)=>({...row,index}));
  }
  function windowRows(rows,start,end,filter='all') {return rows.filter(r=>(filter==='all'||r.source===filter)&&(!r.start&&!r.end||((r.start||r.end)<=end&&(r.end||r.start)>=start)));}
